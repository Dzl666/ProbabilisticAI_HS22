...
def mlp(sizes, activation, output_activation=nn.Identity):
    ...
    Parameters
    ----------
    sizes: List of feature sizes, i.e., [indput_dim, hidden_layer_1, ..., hidden_layer_n_dim, output_dim] 

    activation: nn.Module - Activation function for the hidden layers.
    ...
    # print(mlp_model)
    # 8-64 Tanh 64-64 Tanh 64-4 Identity
    return mlp_model
...
class Actor(nn.Module):
    ...
    def __init__(self, obs_dim, act_dim, hidden_sizes, activation):
        super().__init__()
        # The logits_net returns for a given observation the log probabilities.
    ...
    def _distribution(self, obs):
        ...
        # p for each elememt equals 1 in a 0-1 vector
        return torch.distributions.Categorical(logits=self.logits_net(obs))

    def _log_prob_from_distribution(self, pi:torch.distributions.Distribution, act):
        """Take a distribution and action, then gives the log-probability of the action under that distribution.
    ...
    def forward(self, obs, act=None):
        ...
        log_p_act = None if act is None else self._log_prob_from_distribution(pi, act)
...
class VPGBuffer:
    ...
    def store(self, obs, act, rew, val, logp):
        """Append a single timestep to the buffer. This is called at each environment update to store the observed outcome in
            self.obs_buf, self.act_buf, self.rew_buf, 
            self.val_buff, self.logp_buff.
    ...
    def end_traj(self, last_val=0):
        ...
        ----------
        last_val: np.float32
            Last value is value(state) if the rollout is cut-off at a certain state, or 0 if trajectory ended uninterrupted.
        """

        # Get the indexes where TD residuals and discounted 
        # rewards-to-go are stored.
        path_slice = slice(self.path_start_idx, self.ptr)
        # Observed rewards and Predicted values
        ...
        # TODO: TD residuals calculation.
        d_vals = vals[1:]-vals[:-1]
        self.tdres_buf[path_slice] = discount_cumsum(rews[:-1]+d_vals, self.gamma* self.lam)

        # TODO: discounted rewards-to-go calculation. 
        # trajectory's remaining return
    ...
    def get(self):
        ...
        data = dict(
            obs=self.obs_buf, act=self.act_buf, ret=self.ret_buf, tdres=self.tdres_buf, logp=self.logp_buf
        )
...
class Agent:
    ...
    def step(self, state):
        """Take an state and return action, value function, and log-likelihood of chosen action.
        ...
        # TODO: Implement this function.
        with torch.no_grad():
            # prob. distribution for actions
            pi = self.actor._distribution(state)
            act_sample = pi.sample()
            v = self.critic.forward(state)
            # given obs 'state' and the prob. distri. generate from that, get the log_p of the sampled action
            log_p_act = self.actor._log_prob_from_distribution(pi, act_sample)

        return np.array(act_sample), np.array(v), np.array(log_p_act)
...
def train(env, seed=0):
    ...
    # The observations are 8 dimensional vectors, and the actions are numbers, i.e. 0-dimensional vectors (hence act_dim is an empty list).
    obs_dim = [8]
    act_dim = []
    ...
    # ========= Training parameters ========== TODO: change them
    ...
    for epoch in range(epochs):
        ...
        # This is the end of an epoch, so here is where you likely want to update the actor and / or critic function.
        data = buf.get()
        obs = data['obs']
        act = data['act']
        ...
        # policy gradient - residual of TD multi the log_p of action
        log_p_act = agent.actor.forward(obs,act)[1]
        actor_loss = - tdres @ log_p_act
        actor_loss.backward()
        ...
        for _ in range(100):
            ...
            val_estimate = agent.critic.forward(obs)
            critic_loss = nn.functional.mse_loss(val_estimate, ret)
