...
# TODO: Reliability_diagram_1. 
# Set `EXTENDED_EVALUATION` to `True` in order to visualize your predictions.
EXTENDED_EVALUATION = False

# Result and hyperparameter
# method        ece         acc         score
# Backprop
#
#
#

# docker build --tag task2 .; docker run --rm -v "$(pwd):/results" task2
...
def run_solution(dataset_train: torch.utils.data.Dataset, data_dir: str = os.curdir, output_dir: str = '/results/'):
    ...
    Make sure to preserve the method signature and to return your trained model, else the checker will fail!
...
def calc_calibration_curve(predicted_probabilities: np.ndarray, labels: np.ndarray, num_bins=30)-> dict:
    ...
    for bin_i, (bin_lower, bin_upper) in enumerate(zip(bin_lowers, bin_uppers)):
        ...
        calib_confidence[bin_i] = (bin_lower + bin_upper)/2
        index = np.argwhere((confidences > bin_lower) & (confidences < bin_upper))
        acc_sum = 0
        for i in range(len(index)):
            acc_sum += accuracies[i]
        if len(index):
            calib_accuracy[bin_i] = acc_sum / len(index)
        else:
            calib_accuracy[bin_i] = calib_confidence[bin_i]
        ratios[bin_i] = calib_accuracy[bin_i] / calib_confidence[bin_i]
...
class Framework(object):
    ...
    def predict(self, data_loader: torch.utils.data.DataLoader) -> np.ndarray:
        ...
        :return: (num_samples, 10) NumPy float array where the 2nd dimension sums up to 1 for each row
        """
        probability_batches = []
        
        for batch_x, _ in tqdm.tqdm(data_loader):
            # get pridiction of each image size-(1, 10)
    ...
    def train(self):
        ...
        for _ in progress_bar:
            for batch_idx, (batch_x, batch_y) in enumerate(self.train_loader):
                # batch_x are of shape (batch_size, 784), batch_y are of shape (batch_size, 1)
...
class MNISTNet(nn.Module):
    def __init__(self, in_features: int, out_features: int,
                dropout_p=0.1, dropout_at_eval=True):
        super().__init__()
        ...
        self.activation_func = nn.ReLU() #nn.LeakyReLU(0.01)

    def forward(self, x):
        # TODO General_2: Play around with the network structure
        # You might add different modules like Pooling 
        x = F.dropout(
            self.activation_func(self.layer1(x)),
            p=self.dropout_p,
            training=self.training or self.dropout_at_eval
        )
        x = F.dropout(
            self.activation_func(self.layer2(x)),
            p=self.dropout_p,
            training=self.training or self.dropout_at_eval
        )
...
class DropoutTrainer(Framework):
    def __init__(self, dataset_train,
        ...
        self.network = MNISTNet(in_features=28 * 28, out_features=10, dropout_at_eval=True)
        self.train_loader = torch.utils.data.DataLoader(
            dataset_train, batch_size=self.batch_size, shuffle=True, drop_last=True
        )
        self.optimizer = torch.optim.SGD(self.network.parameters(), lr=self.learning_rate, weight_decay=self.weight_decay)
    ...
    def train(self):
        ...
        for _ in progress_bar:
            for batch_idx, (batch_x, batch_y) in enumerate(self.train_loader):
                # batch_x - (batch_size, 784), batch_y - (batch_size,)

                self.network.zero_grad()
                # TODO: MC_Dropout_2. Implement MCDropout training here
                current_logits = self.network(batch_x)

                tau = self.network.dropout_p * (self.prior_lenght_scale ** 2) / (2*self.batch_size* self.weight_decay)
                # negative log likelihood + log softmax
                loss = F.nll_loss(F.log_softmax(current_logits, dim=1), batch_y, reduction='sum') / (tau*self.batch_size)
    ...
    def predict_probabilities(self, x: torch.Tensor, num_sample=100) -> torch.Tensor:
        ...
        probability_samples = torch.stack([F.softmax(self.network(x), dim=1) for _ in range(num_sample)], dim=0)
...
class BackpropTrainer(Framework):
    def __init__(self, dataset_train,  *args, **kwargs):
        ...
        self.hidden_features=(128, 128)
        self.batch_size = 128
        self.learning_rate = 1e-3
        self.num_epochs = 100
        self.noise_scale = torch.tensor(1e-4)
    ...
    def train(self):
        ...
        for _ in progress_bar:
            ...
            for batch_idx, (batch_x, batch_y) in enumerate(self.train_loader):
                ...
                output = current_logits.argmax(axis=1)
                for i in range(batch_y.shape[0]):
                    # print()
                    log_likelihood += Normal(output[i], self.noise_scale).log_prob(batch_y[i])
                # use the negative elbo as loss function
                loss = (log_var_post - (log_prior+log_likelihood))
                # Backpropagate to get the gradients
                loss.backward()
                self.optimizer.step()
...
class BayesNet(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> typing.Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        for layer_idx in range(len(self.layers)):
            x, layer_log_prior, layer_log_var_posterior = self.layers[layer_idx](x)
            log_prior += layer_log_prior
            log_variational_posterior += layer_log_var_posterior
...
class BayesianLayer(nn.Module):
    ...
    def __init__(self, in_features: int, out_features: int, bias: bool = True):
        ...
        :param in_features: Number of input features
        :param out_features: Number of output features
        :param bias: If true, use a bias term
        """
        super().__init__()
        ...
        # Background Pytorch will backpropogate gradients to an object initialized with torch.Parameter(...) 
        # and the object will be updated when computing loss.backwards() during training.
        # This will not happen for a torch.Tensor(...) object, which is by default a constant. 

        # TODO: Backprop_1-1. Create a suitable prior for weights and biases as an instance of ParameterDistribution.
        #  You can use the same prior for both weights and biases, but are also free to experiment with other priors.
        #  You can create constants using torch.tensor(...). Do NOT use torch.Parameter(...) here since the prior should not be optimized!

        # maybe conbine two Guassian density
        mu_1 = torch.tensor(0.0)
        sigma_1 = torch.tensor(1.0)
        mu_2 = torch.tensor(2.0)
        sigma_2 = torch.tensor(4.0)
        self.prior = UnivariateGaussian(mu_1, sigma_1) # + UnivariateGaussian(mu_2, sigma_2)
        ...
        # TODO: Backprop_1-2. Create a suitable variational posterior for weights as an instance of ParameterDistribution.
        #  You need to create separate ParameterDistribution instances for weights and biases, but you can use the same family of distributions for each if you want.
        #  IMPORTANT: Create a nn.Parameter(...) for each parameter and add them as an attribute in the ParameterDistribution instances.
        #
        ...
        #  )
        self.weigth = None
        self.weigth_mu = torch.nn.Parameter(torch.zeros(out_features, in_features))
        self.weigth_rho = torch.nn.Parameter(torch.zeros(out_features, in_features))
        ...
        if self.use_bias:
            # TODO: Backprop_1-3. Similarly as you did above for the weights, create the bias variational posterior instance here.
            ...
            self.bias_rho = torch.nn.Parameter(torch.zeros(out_features))
    ...
    def forward(self, inputs: torch.Tensor):
        ...
        self.weights = self.weights_var_posterior.sample() # mu + log(1 + exp(rho)) * eps
        weight_prior = self.prior.log_likelihood(self.weights)
        weight_posterior = self.weights_var_posterior.log_likelihood(self.weights)

        self.bias = None
        bias_prior = None
        bias_posterior = None
        if self.use_bias:
            self.bias = self.bias_var_posterior.sample()
            bias_prior = self.prior.log_likelihood(self.bias)
            bias_posterior = self.bias_var_posterior.log_likelihood(self.bias)

        log_prior = torch.sum(weight_prior) + torch.sum(bias_prior)
        log_variational_posterior = torch.sum(weight_posterior) + torch.sum(bias_posterior)

        return F.linear(inputs, self.weights, self.bias), log_prior, log_variational_posterior
...
class MultivariateDiagonalGaussian(ParameterDistribution):
    ...
    def log_likelihood(self, values: torch.Tensor) -> torch.Tensor:
        ...
        return Normal(self.mu.data, torch.log(1+torch.exp(self.rho))).log_prob(values)

    def sample(self) -> torch.Tensor:
        # TODO: Backprop_5. You need to complete the sample function 
        # for the Multivariate DiagonalGaussian Gaussian distribution.
        # page 4 - section 3.2
        eps = Normal(0,1).sample(self.mu.shape)
        return self.mu + torch.log(1+torch.exp(self.rho)) * eps
