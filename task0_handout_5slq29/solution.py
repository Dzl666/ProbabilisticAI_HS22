...
def log_posterior_probs(x):
    ...
    Returns:
        log_posterior_probs (np.ndarray): a numpy array of size 3, containing the Bayesian log-posterior probabilities corresponding to the three hypotheses
    """
    assert x.ndim == 1
    log_p = np.zeros(3)
    for h_cnt in range(3):
        log_p[h_cnt] = np.sum(np.log(HYPOTHESIS_SPACE[h_cnt].pdf(x))) + np.log(PRIOR_PROBS[h_cnt])
